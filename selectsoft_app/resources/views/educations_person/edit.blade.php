...
<header class="header">
    ...
    <main class="container">
        <form action="{{route('educations.update', ['education_person' => $education->id])}}" method="post">
            @csrf
            @method('PATCH')
            <label for="school_name">Institucion educativa: </label><br>
            <input type="text" placeholder="institucion" name="shcool_name" value="{{old('shcool_name')}} {{$education->shcool_name}}"><br>
            @error('shcool_name')
                <span style="color:red;">{{$message}}</span><br>
            @enderror
            <label for="obtained_title">TItulo obtenido: </label><br>
            <input type="text" name="obtained_title" placeholder="Titulo obtenido" value="{{old('obtained_title')}} {{$education->obtained_title}}">
